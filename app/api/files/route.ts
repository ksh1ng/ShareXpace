...
export async function POST(request: Request) {
  return withRequestedWorkspaceResponse(request, "The file could not be uploaded.", async () => {
    ...
    await FILES.put(objectKey, file.stream(), { httpMetadata: { contentType } });
    const createdAt = new Date().toISOString();
    const expiresAt = expiresAtFor("static", new Date(createdAt));
    await DB.batch([
      DB.prepare("INSERT INTO workspace_files (id, workspace_id, name, content_type, size, object_key, author, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
    ...
    return Response.json({
      record: {
        id: `memory-${id}`, kind: "file", title: file.name, detail: `${contentType} · ${(file.size / 1024).toFixed(0)} KB`,
        ...
        supersededBy: null, sourceUrl: null, summary: `Member-uploaded workspace file: ${file.name}`, version: 1, stale: false,
      },
      knowledgeVersion,
    }, { status: 201 });
  });
