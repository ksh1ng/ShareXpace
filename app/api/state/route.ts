  ...
  return withRequestedWorkspaceResponse(request, "Unable to load workspace.", async () => {
    ...
    return Response.json({
      records: state.records.map((record, index) => ({
        ...
        searchedAt: record.generated_at ?? record.created_at,
        cacheHitCount: record.cache_hit_count ?? 0,
        ragReuseCount: record.rag_reuse_count ?? 0,
        lastHitAt: record.last_hit_at,
        lastHitTime: record.last_hit_at ? relativeTime(record.last_hit_at) : null,
      })),
      files: state.files,
