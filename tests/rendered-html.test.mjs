...
test("demo guide includes beginner Codex MCP setup and verification", async () => {
  ...
  assert.match(guide, /## 4\. Codex App demo/);
  assert.match(guide, /## 5\. Codex CLI demo/);
  assert.match(guide, /Codex → Plugins/);
  assert.match(guide, /ShareXpace/);
  assert.match(guide, /Open \*\*Codex → Plugins\*\*/);
  assert.match(guide, /Tasks opened before installation do not automatically load/);
  ...
  assert.match(guide, /Do members need `OPENAI_API_KEY`\?/);
  assert.doesNotMatch(guide, /[\u3400-\u9fff]/);
  assert.doesNotMatch(guide, /team-memory-production|`\.\.\/team-memory`/i);
