...
- Route precedence is `source refresh → explicit post-hit RAG revision → no/low match → fresh exact Semantic Cache → forced RAG for non-exact related knowledge → threshold routing`. Threshold routing accepts a high Hybrid score, raw embedding score of at least 80%, or normalized lexical score of at least 88% when freshness allows it. This prevents wording differences from diluting a strong paraphrase into RAG while keeping the noisier lexical-only route conservative.
