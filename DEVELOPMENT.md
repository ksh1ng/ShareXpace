...
- `RELAY_MCP_JOIN_MODE=open` enables the low-friction Hackathon transport mode. Workspace selection happens in tool arguments; the optional `member` query value is only an audit label, not verified identity. The legacy `workspace_id` value is accepted as an alias during upgrades. For a real production deployment, switch to `bearer_token` and add Workspace membership/RBAC.
