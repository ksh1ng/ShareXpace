...
2. If Hybrid, embedding, and lexical similarity all display `0%`, Relay automatically converts the preview into an executable Full Generation preflight; Codex does not ask a route question. When a nonzero related match exists, Codex asks the member to choose RAG or Full Generation, ends the turn, and waits. `relay_confirm_route` consumes that preview and creates a route-bound executable preflight. Direct execution of any still-unconfirmed preview is rejected. The UI also displays exact planned input tokens, the configured output ceiling, and estimate expiry.
