...
- Agent 可呼叫 `relay_create_workspace` 建立新的 Workspace，取得新 MCP URL 後讓其他 Agent 加入。
...
每個 MCP URL 的 `workspace_id` 會選定獨立的 D1 partition；memory、embeddings、chat、cache 與 analytics 不會跨 Workspace。Dashboard 目前顯示預設的 `RoamTogether`，新增 Workspace 主要透過 MCP Agent 操作。上傳到 R2 的檔案目前只保存 bytes 與 metadata，尚未自動解析及建立文件 chunks。
...
| Workspace ID | `RoamTogether` | 加入預設 Workspace 所需的唯一值 |

使用者把 Dashboard 顯示的 ID 放進 MCP URL即可加入，不需要 Member token。連線後應呼叫 `relay_get_workspace`，確認回傳 ID 相同；要建立其他 Workspace 時使用 `relay_create_workspace`。
...
```bash
codex mcp add relay \
  --url "https://relay-production-2026.opompm841218.chatgpt.site/api/mcp?workspace_id=RoamTogether&member=Alice"
```
...
### 3.6 透過 Agent 建立新 Workspace

在已連上 `RoamTogether` 的 Codex 對話輸入：

```text
請使用 Relay MCP 的 relay_create_workspace 建立一個新的共享 Workspace：
name: ProductLaunch
workspaceId: ProductLaunch
建立後只回報 Workspace name、Workspace ID 和 MCP path。
```
...
Connect another MCP client with: /api/mcp?workspace_id=ProductLaunch&member=<display-name>
```

接著可另建一個 MCP 設定，讓該 Agent 進入新 Workspace：

```bash
codex mcp add relay-product-launch \
  --url "https://relay-production-2026.opompm841218.chatgpt.site/api/mcp?workspace_id=ProductLaunch&member=Alice"
```

重新開啟 Codex session 後呼叫 `relay_get_workspace`，應回傳 `ProductLaunch`。新 Workspace 的 knowledge、embeddings、chat、cache 與 analytics 都是空白且獨立的。
...
2. URL 是否包含 `/api/mcp?workspace_id=RoamTogether`。
...
```text
URL: https://<relay-host>/api/mcp?workspace_id=RoamTogether&member=Alice
```
...
```bash
curl "https://<relay-host>/api/mcp?workspace_id=RoamTogether&member=Alice" \
...
使用回傳的 `mcpPath` 建立另一個 MCP client connection，該連線之後的所有工具與 resources 都會使用新 Workspace partition。
