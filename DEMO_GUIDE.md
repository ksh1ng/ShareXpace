...
- Codex、ChatGPT、IDE Agent 或其他 MCP Client 可連接 `/mcp`。
...
Web Dashboard 使用開發伺服器顯示的 URL。MCP endpoint 是相同 host 下的 `/mcp`。
...
- `DEMO_GUIDE.md`：本文件。
- `app/mcp/route.ts`：MCP tools/resources 與 JSON-RPC transport。
