...
| --- | --- | --- | --- |
| **Codex App 版** | 評審、影片、非技術觀眾 | 安裝 Relay Shared Workspace Plugin | 3 分鐘 |
...
本專案已把 Relay MCP endpoint、Workspace workflow 與三層路由規則包成 **Relay Shared Workspace** Codex plugin。這是 Demo 的首選安裝方式；不需要手動執行 `codex mcp add`、編輯 `~/.codex/config.toml`、準備模型 API key 或 Relay Member token。

1. 先安裝並登入 Codex App，然後用 Codex 開啟本專案資料夾。
2. 完整關閉並重開 Codex App，讓它讀取 `.agents/plugins/marketplace.json`。
3. 打開 **Plugins**，選擇 **Relay Build Week**。
4. 找到 **Relay Shared Workspace**，按一次 **Install**。
...
4. 進入 **Codex → Plugins**，選擇 **Relay Build Week**。
5. 打開 **Relay Shared Workspace**，按 **Install**。確認看到藍紫色雙手托住 AI Workspace 的 Plugin icon。
...
CLI 有兩種可行方式：在專案根目錄啟動 `codex`，用 `/plugins` 從 **Relay Build Week** 安裝 Plugin；或使用下方 installer 直接註冊 MCP。單人展示可用 Plugin，Alice／Bob 需要不同 Dashboard audit label 時，使用 installer 分別輸入 `Alice`、`Bob` 最清楚。
