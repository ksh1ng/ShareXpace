...
PLUGIN_NAME="sharexspace"
MARKETPLACE_NAME="sharexspace"
...
usage() {
  cat <<'EOF'
ShareXspace plugin installer

Usage:
  ./INSTALL_SHAREXSPACE_PLUGIN.command [--check-only] [--skip-handshake]
...
fail() {
  printf '\nShareXspace plugin setup failed: %s\n' "$1" >&2
  exit 1
}
...
for required in \
  ...
  "$PLUGIN_DIR/.mcp.json" \
  "$PLUGIN_DIR/skills/sharexspace-workspace/SKILL.md" \
  "$PLUGIN_DIR/assets/sharexspace-icon.png"; do
...
grep -q '"name"[[:space:]]*:[[:space:]]*"sharexspace"' "$MARKETPLACE_FILE" || fail "Marketplace name is invalid."
grep -q '"name"[[:space:]]*:[[:space:]]*"sharexspace"' "$PLUGIN_DIR/.codex-plugin/plugin.json" || fail "Plugin manifest name is invalid."
...
if [ "$SKIP_HANDSHAKE" -eq 1 ]; then
  printf 'ShareXspace plugin package is valid; live MCP verification was skipped.\n'
else
  printf 'ShareXspace plugin package and hosted MCP endpoint are valid.\n'
fi
...
if codex plugin marketplace list 2>/dev/null | grep -Fq "$ROOT_DIR"; then
  printf 'ShareXspace marketplace is already registered.\n'
else
...
ShareXspace is installed.

1. Restart the ChatGPT desktop app or Codex CLI.
2. Start a new Codex task/session.
3. Send: Set up ShareXspace and show available workspaces.
4. Open the Dashboard: https://relay-production-2026.opompm841218.chatgpt.site

The ShareXspace hosted Relay supplies D1, R2, embeddings, cache, and Dashboard storage.
